...
class StatePlot:
    ...
    def add_state(self, name, sigma_bounds=None, max_length=None, is_angle=False, rad2deg=False):
        self.states[name] = StateData(sigma_bounds=sigma_bounds, max_length=max_length, is_angle=is_angle,
                                      rad2deg=rad2deg)
        ...
        if self.dimension == 1:
            ...
            if self.has_sigma:
                for bound in self.sigma_bounds:
                    ...
                    self.sigma_plots[bound]['lower_plot'].setData(x_data, state_objs[0].sigma_data[bound]['lower'],
                                                                  pen=pen)
                    self.sigma_plots[bound]['upper_plot'].setData(x_data, state_objs[0].sigma_data[bound]['upper'],
                                                                  pen=pen)
