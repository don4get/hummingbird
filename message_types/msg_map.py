...
class msg_map:
    def __init__(self, PLAN):
        ...
        # indexed as [index_E, index_N]
        self.building_height = PLAN.building_height * np.random.rand(PLAN.num_blocks, PLAN.num_blocks)
        # alternate array indexed as [index_N, index_E]
        self.bldg_heights = self.building_height.T
        # the width of the buildings (all the same)
        self.building_width = np.ceil(PLAN.city_width / PLAN.num_blocks * (1 - PLAN.street_width))
